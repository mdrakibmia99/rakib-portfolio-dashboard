...
export const sendEmail = async (to: string, orderId: string, status: string) => {
    const transporter = nodemailer.createTransport({
        ...
        secure: false,
        auth: {
            user: 'rkrakibhasan680@gmail.com',
            pass: 'xaxg yrah nahs uiqq',
        },
    });
    ...
    const htmlTemplate = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border-radius: 10px; box-shadow: 0px 4px 10px rgba(0,0,0,0.1);">
            ...
            <p style="font-size: 16px; color: #333; line-height: 1.6;">
                ...
                <br><br>
                Thank you for shopping with Glaazex!
            </p>
            <hr style="border: none; height: 1px; background: #ddd; margin: 20px 0;">
            <p style="font-size: 14px; color: #777; text-align: center;">
                Best Regards, <br>
                <strong>Glaazex Team</strong>
            </p>
        </div>
    `;

    await transporter.sendMail({
        from: `"Glaazex" rkrakibhasan680@gmail.com`,
        to,
        subject,
        text: `Your order #${orderId} status is now ${status}. Thank you for choosing glaazex!`, // plain text fallback
        html: htmlTemplate,
    });
};
